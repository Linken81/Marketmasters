...
// - Achievements & shop reset on refresh; achievements stored as booleans (no dates).
// - XP progress bar, confetti on level/achievement unlock.
// - Missions: 3 active, replaced on claim.
// - First Trade achievement unlocks on first buy or sell.
// - Defensive DOM guards so missing elements won't abort the script.
// - Chart updates, market ticks every 10s, news every 3 minutes.
...
// Per user's request: reset achievements and shop and level/XP on refresh
state.level = 1;
state.xp = 0;
state.achievements = {};
state.shopOwned = {};
...
function unlockAchievement(id) {
  ...
  state.achievements[id] = true; // store boolean flag (no date)
  if (spec) {
