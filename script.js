// script.js — updated: missions UI fixes (title -> "Missions", reward display, reliable claim handler).
// Also includes existing game systems: XP/levels, achievements, shop, prestige, watchlist, chart, ticks, news.
// Defensive guards added; chart initialized after DOMContentLoaded.
...
// Normalize achievements to canonical shape so UI shows Locked/Unlocked correctly
...
// Per request: reset level to 1 and xp on refresh
state.level = 1;
state.xp = 0;
saveState();
...
function addXP(amount){
  ...
  checkLevelUp(); saveState(); updateHUD();
}
...
function updateHUD(){
  ...
  if(bar){ const pct = Math.min(100, Math.round((state.xp / xpForLevel(state.level)) * 100)); bar.style.width = pct + '%'; }
  updateCash(); renderNextAchievement();
}
...
function unlockAchievement(id, note=''){
  ...
  if(spec){ state.coins = (state.coins||0) + (spec.coins||0); toast(`Achievement unlocked: ${spec.name} (+${spec.coins||0} coins)`); launchConfetti(80); }
...
function renderMissionsModal(){
  ...
  modalList.innerHTML = '';
  const activeCount = (state.missions || []).length;
  // title text in modal is handled at DOMContentLoaded (rename), so just render items here
  (state.missions || []).forEach((m, idx) => {
    ...
    div.innerHTML = `<div style="max-width:70%">
        ...
        <div style="font-size:0.9em;color:#9aa7b2">${rewardText}</div>
        <div class="meta" style="font-size:0.9em;color:#9aa7b2">${statusText}</div>
      </div>
...
// central claim handler used by delegated click listener
function claimMission(index){
  ...
  // replace mission slot with a new one (or remove)
...
// brief missions list displayed under reserved panel
function renderMissionsBrief(){
  const el = document.getElementById('missions-brief');
  if(!el) return;
  el.innerHTML = '';
  (state.missions || []).slice(0,3).forEach(m => {
    const d = document.createElement('div'); d.textContent = `${m.text}${m.done ? ' ✅' : ''}`; el.appendChild(d);
  });
}
...
function triggerRandomNews(){ const news = NEWS_EVENTS[Math.floor(Math.random()*NEWS_EVENTS.length)]; const el = document.getElementById('news-content'); if(el) el.textContent = news.text; const newsMap = {}; if(news.type==='stock') newsMap[news.symbol]=news.effect; else if(news.type==='type') STOCKS.forEach(s=>{ if(s.type===news.target) newsMap[s.symbol]=news.effect; }); else STOCKS.forEach(s=>newsMap[s.symbol]=news.effect); if(news.mood==='good') addXP(5 + Math.round(Math.abs(news.effect) * 100)); if(news.mood==='bad') addXP(2); addEventToList(news.text); return newsMap; }
...
let orderHistory=[]; function recordOrder(type,symbol,qty,price){ orderHistory.unshift({type,symbol,qty,price,ts:new Date().toISOString()}); if(orderHistory.length>200) orderHistory.pop(); }
...
window.buyStock = function(symbol){ const input=document.getElementById(`buy_${symbol}`); let qty=input?parseInt(input.value):1; qty=Math.max(1, qty||1); const cost=(prices[symbol]||0)*qty; if(cost<=portfolio.cash){ const prevQty=portfolio.stocks[symbol]||0; const totalQty=prevQty+qty; averageBuyPrice[symbol] = (averageBuyPrice[symbol]*prevQty + prices[symbol]*qty)/Math.max(1,totalQty); portfolio.cash -= cost; portfolio.stocks[symbol] = totalQty; dayProgress.trades=(dayProgress.trades||0)+1; if(!dayProgress.typesBought) dayProgress.typesBought=[]; const type=(STOCKS.find(s=>s.symbol===symbol)||{}).type; if(type && !dayProgress.typesBought.includes(type)) dayProgress.typesBought.push(type); if(!state.firstTradeDone){ state.firstTradeDone=true; saveState(); } addXP(Math.max(1, Math.round(cost/200))); state.coins += Math.max(0, Math.round(cost/1000)); recordOrder('buy', symbol, qty, prices[symbol]); toast(`Bought ${qty} ${symbol} for ${formatCurrency(cost)}`); saveState(); updateHUD(); updateCash(); updatePortfolioTable(); updateTradeTable(); updateStockTable(); renderWatchlist(); checkMissions(); checkAchievements(); } else toast('Not enough cash'); };

window.sellStock = function(symbol){ const input=document.getElementById(`sell_${symbol}`); let qty=input?parseInt(input.value):1; qty=Math.max(1, qty||1); const owned=portfolio.stocks[symbol]||0; if(qty>owned){ toast('Not enough shares'); return;} const revenue=(prices[symbol]||0)*qty; portfolio.cash += revenue; portfolio.stocks[symbol] = owned - qty; if(portfolio.stocks[symbol]===0){ averageBuyPrice[symbol]=0; prevOwned[symbol]=0;} const profit=(prices[symbol]-averageBuyPrice[symbol])*qty; if(profit>0){ addXP(Math.round(profit/10)); state.coins += Math.round(profit/50); dayProgress.dayProfit=(dayProgress.dayProfit||0)+profit; state.cumulativeProfit=(state.cumulativeProfit||0)+profit; } recordOrder('sell', symbol, qty, prices[symbol]); toast(`Sold ${qty} ${symbol} for ${formatCurrency(revenue)}`); saveState(); updateHUD(); updateCash(); updatePortfolioTable(); updateTradeTable(); updateStockTable(); checkMissions(); checkAchievements(); };
...
function tickPrices(){ try { setRandomPrices({}); updateStockTable(); updateTradeTable(); updatePortfolioTable(); pushChartSample(getPortfolioValue()); tickCount++; STOCKS.forEach(s=>{ if(portfolio.stocks[s.symbol]>0) dayProgress.holdTicks=(dayProgress.holdTicks||0)+1; }); checkMissions(); checkAchievements(); updateHUD(); } catch(e){ console.error('tickPrices error', e); } }
function newsTick(){ try { const map = triggerRandomNews(); setRandomPrices(map); updateStockTable(); updateTradeTable(); updatePortfolioTable(); pushChartSample(getPortfolioValue()); renderLeaderboard(); saveState(); } catch(e){ console.error('newsTick error', e); } }

// ------------------ Missions check ------------------
function checkMissions(){ dayProgress.buyDifferent = Object.values(portfolio.stocks).filter(v=>v>0).length; dayProgress.trades = dayProgress.trades || 0; dayProgress.typesBought = dayProgress.typesBought || []; (state.missions||[]).forEach(m=>{ if(m.done) return; try{ if(typeof m.check === 'function' ? m.check(dayProgress) : false) m.done = true; else { const candidate = MISSION_CANDIDATES.find(c=>c.id===m.id); if(candidate && candidate.check(dayProgress)) m.done = true; } } catch(e){ if(m.id==='buy_3' && dayProgress.buyDifferent>=3) m.done=true; if(m.id==='profit_500' && (dayProgress.dayProfit||0)>=500) m.done=true; if(m.id==='hold_10' && (dayProgress.holdTicks||0)>=10) m.done=true; if(m.id==='trade_10' && (dayProgress.trades||0)>=10) m.done=true; if(m.id==='buy_food' && (dayProgress.typesBought||[]).includes('Food')) m.done=true; } }); renderMissionsModal(); }

// ------------------ Helpers ------------------
function getPortfolioValue(){ let total = (portfolio && portfolio.cash) ? portfolio.cash : 0; for(let i=0;i<STOCKS.length;i++){ const s=STOCKS[i], sym=s&&s.symbol; if(!sym) continue; const owned=(portfolio.stocks&&portfolio.stocks[sym])?portfolio.stocks[sym]:0; const p=(prices&&prices[sym]!==undefined)?prices[sym]:0; total += owned * p; } return +total; }
function setRandomPrices(newsEffectMap={}){ prevPrices = {...prices}; STOCKS.forEach(stock=>{ let old = prices[stock.symbol]||randomPrice(); let change=(Math.random()*0.07)-0.035; if(Math.random()<0.10) change += (Math.random()*0.06-0.03); if(newsEffectMap[stock.symbol]) change += newsEffectMap[stock.symbol]; change = Math.max(-0.5, Math.min(0.5, change)); prices[stock.symbol] = Math.max(5, +(old*(1+change)).toFixed(2)); }); }

// ------------------ DOMContentLoaded startup ------------------
document.addEventListener('DOMContentLoaded', ()=> {
  // rename modal title from "Daily Missions" to "Missions" (user request)
  const modalM = document.getElementById('modal-missions');
  if(modalM){
    const h3 = modalM.querySelector('h3');
    if(h3) h3.textContent = 'Missions';
    // delegate claim button clicks inside modal (reliable)
    modalM.addEventListener('click', (ev) => {
      const btn = ev.target.closest('button[data-claim]');
      if(btn){
        const idx = btn.getAttribute('data-claim');
        claimMission(idx);
      }
    });
  }
...
window.onerror = function(message, source, lineno, colno, error){
  ...
  try { const toasts=document.getElementById('toasts'); if(toasts){ const el=document.createElement('div'); el.className='toast'; el.textContent=`Error: ${message} (line ${lineno})`; toasts.appendChild(el); setTimeout(()=>el.remove(),8000); } } catch(e){}
  return false;
};
