// script.js — Achievements improvements: Locked/Unlocked fixed and additional achievements added.
// - Normalizes loaded achievements to canonical shape.
// - Achievements modal always lists all achievements and shows "Locked" until unlocked.
// - Added new achievements and their auto-check conditions.
// - Ensures buy/sell/trade counters and flags are persisted so achievement checks work after reload.
// - Keeps missions, XP, shop, prestige, watchlist, confetti, chart and other game systems intact.
...
let state = {
  ...
  level: 1,
  coins: 0,
  achievements: {},    // canonical: { id: { unlockedAt, note } }
  missions: [],
  missionsDate: null,
  shopOwned: {},
  ...
  firstSaleDone: false,
  tradeCount: 0,       // persisted count of trades (buys + sells)
  totalCoinsEarned: 0  // cumulative coins gained (optional achievement)
};
...
// Normalize achievements to canonical object form so UI shows Locked/Unlocked correctly
...
// Reset level/XP on reload per user request
state.level = 1;
state.xp = 0;
saveState();
...
const ACHIEVEMENT_LIST = [
  ...
  { id:'trade_10', name:'Active Trader', desc:'Execute 10 trades (buy or sell)', coins:80 },
  { id:'own_5', name:'Collector', desc:'Own 5 different stocks simultaneously', coins:120 },
  { id:'rich_1000c', name:'Well Funded', desc:'Have 1000 coins total (in-game coins)', coins:150 }
];
  ...
  ACHIEVEMENT_LIST.forEach(a => {
    ...
    div.innerHTML = `
      <div>
        ...
        <div style="font-size:0.9em;color:#9aa7b2">${a.desc}</div>
      </div>
      <div style="font-weight:700; color:${color}">${unlockedText}</div>
    `;
    el.appendChild(div);
  });
}
...
function checkAchievements(){
  ...
  if (!state.achievements['hold_100ticks'] && (dayProgress.holdTicks || 0) >= 100) unlockAchievement('hold_100ticks');
  // level
...
// ------------------ Missions (unchanged logic, but claim UI fixed below) ------------------
...
// Delegated mission claim handler (wired at DOMContentLoaded)
...
function renderShop(){
  const el = document.getElementById('shop-items'); if(!el) return; el.innerHTML='';
  SHOP_ITEMS.forEach(item => {
    ...
    if (!owned){
      ...
      if (btn) btn.onclick = () => {
        if (state.coins >= item.price){ state.coins -= item.price; state.shopOwned[item.id] = true; applyShopEffect(item); toast(`Purchased ${item.name}`); saveState(); updateHUD(); renderShop(); } else toast('Not enough coins');
      };
    }
  });
}
...
document.addEventListener('DOMContentLoaded', ()=> {
  ...
  // initialize chart (safe after DOM ready)
...
// delegated claim wrapper for safety (calls the shared claimMission logic)
function claimMission(index){
  const idx = Number(index);
  if (Number.isNaN(idx)) return;
  const m = state.missions[idx];
...
// global error hook to help debugging
