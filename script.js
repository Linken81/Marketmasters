// script.js
// Fixed tickCount ReferenceError and hardened startup/order of initialization.
// Ensures dayProgress, tickCount and helpers exist before any logic runs.
// Missions are generated and checked after buys/sells and on tick updates.
// Keep hard-refresh after replacing file to ensure browser loads the new script.
...
// ------------------ Globals (must be declared early) ------------------
let tickCount = 0; // <--- critical: declared before tickPrices runs
let dayProgress = { buyDifferent:0, dayProfit:0, holdTicks:0, trades:0, typesBought:[] };

// helper used by mission generation
...
// Per your earlier request: reset level to 1 on refresh
state.level = 1;
state.xp = 0;
saveState();
...
function renderMissionsBrief(){
  const el = document.getElementById('missions-brief'); if(!el) return; el.innerHTML = '';
...
// ------------------ Table updates and trading logic ------------------
...
window.sellAllStock = function(symbol){ const owned=portfolio.stocks[symbol]||0; if(owned>0){ const input=document.getElementById(`sell_${symbol}`); if(input) input.value=owned; sellStock(symbol); } };
...
// global error hook to show helpful toast and avoid silent failures
