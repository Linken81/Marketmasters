// Fixed script.js (confetti, XP, missions, achievements, shop, watchlist, order history)
// - Fixed getPortfolioValue() to avoid using an undefined `symbol` identifier
// - Added DOM guards so missing elements don't throw and stop execution
// - Keeps market behaviour (10s ticks, news every 3 minutes), chart sampling, and new features
...
// Per your earlier request: reset level to 1 on page refresh
state.level = 1;
state.xp = 0;
saveState();
...
// ------------------ Other helpers (random prices & chart push) ------------------
function setRandomPrices(newsEffectMap = {}){
  prevPrices = {...prices};
  STOCKS.forEach(stock=>{
    let oldPrice = prices[stock.symbol] || randomPrice();
