// script.js — fully updated, cleaned, and hardened
// - Fixes for previous ReferenceErrors (tickCount, dayProgress, getTodayStr).
// - Normalizes saved achievement data so Achievements modal shows Locked/Unlocked correctly.
// - XP/Leveling, Achievements, Missions (3 active, replace when claimed), Shop, Watchlist, Order History,
//   Confetti animations, Prestige, Leaderboard, News, and Chart remain implemented.
// - Defensive DOM guards and global error handler added.
// - Chart initialization moved to DOMContentLoaded to avoid early DOM access.
...
let tickCount = 0; // avoids ReferenceError in tick loop
...
let state = {
  ...
  level: 1,
  coins: 0,
  achievements: {},     // normalized to { id: { unlockedAt, note } }
  missions: [],
  missionsDate: null,
  shopOwned: {},
...
// Normalize loaded achievements so UI doesn't show everything unlocked due to legacy shapes
function normalizeLoadedState(){
  if (!state.achievements || typeof state.achievements !== 'object') state.achievements = {};
  Object.keys(state.achievements).forEach(k => {
    const v = state.achievements[k];
    if (!v) {
      delete state.achievements[k];
      return;
    }
    if (v === true) {
      // convert legacy boolean true -> object
      state.achievements[k] = { unlockedAt: new Date().toISOString(), note: 'migrated' };
    } else if (typeof v === 'object' && v.unlockedAt) {
...
// Per user request: reset level to 1 and xp when reloading the page
state.level = 1;
state.xp = 0;
saveState();
...
function renderMissionsModal(){
  ...
  (state.missions || []).forEach((m, idx) => {
    ...
    if (m.done) {
      ...
      if (btn) btn.onclick = () => {
        const r = m.reward || { coins:50, xp:15 };
        state.coins += r.coins || 0;
        addXP(r.xp || 0);
        toast(`Mission claimed: +${r.coins} coins, +${r.xp} XP`);
...
function triggerRandomNews(){
  ...
  const newsMap = {};
  if (news.type === 'stock') newsMap[news.symbol] = news.effect;
  else if (news.type === 'type') STOCKS.forEach(s => { if (s.type === news.target) newsMap[s.symbol] = news.effect; });
  else STOCKS.forEach(s => newsMap[s.symbol] = news.effect);
...
function getPortfolioValue(){
  ...
  for (let i=0;i<STOCKS.length;i++){
    const s = STOCKS[i], sym = s && s.symbol;
    if (!sym) continue;
...
// global error hook (useful for debugging)
