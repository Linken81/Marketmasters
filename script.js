...
if(canvas && typeof Chart !== 'undefined'){
  ...
  chartData.datasets[0].data.push(portfolioHistory[0]);
...
function renderWatchlist(){
  ...
  watchlist.forEach(sym => {
    ...
    const price = (prices[sym] !== undefined) ? `$${prices[sym].toFixed(2)}` : '—';
    ...
    const btn = div.querySelector('button');
    if(btn) btn.onclick = ()=> { watchlist = watchlist.filter(s=>s!==sym); renderWatchlist(); };
  });
}
let orderHistory = [];
...
function updateStockTable(){
  ...
  tbody.innerHTML = '';
  STOCKS.forEach(stock => {
    const p = (prices[stock.symbol] !== undefined) ? prices[stock.symbol] : 0;
    const change = +(p - (prevPrices[stock.symbol] || p));
    const changeStr = (change>0?'+':'') + change.toFixed(2);
    const className = change>0 ? 'price-up' : change<0 ? 'price-down' : 'price-same';
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${stock.symbol}</td><td>${stock.type}</td><td>$${p.toFixed(2)}</td><td class="${className}">${changeStr}</td><td></td>`;
...
function updateTradeTable(){
  ...
  STOCKS.forEach(stock => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${stock.symbol}</td><td>${stock.type}</td><td>$${p.toFixed(2)}</td><td class="${className}">${changeStr}</td>
      <td>
        <input type="number" min="1" value="1" class="buy-input" id="${rowId}">
        <button onclick="buyStock('${stock.symbol}')" class="action-btn">Buy</button>
        <span class="buy-cost" id="${costId}">$${p.toFixed(2)}</span>
      </td>`;
    tbody.appendChild(tr);
    setTimeout(()=>{
      const qtyInput = document.getElementById(rowId), costSpan = document.getElementById(costId);
      if(qtyInput && costSpan){ function updateCost(){ let q = parseInt(qtyInput.value)||0; costSpan.textContent = `$${(q*p).toFixed(2)}`; } qtyInput.addEventListener('input', updateCost); updateCost(); }
    }, 0);
  });
}
function updatePortfolioTable(){
  ...
  STOCKS.forEach(stock => {
    ...
    if(owned > 0){
      const p = (prices[stock.symbol] !== undefined) ? prices[stock.symbol] : 0;
      const totalValue = owned * p;
      const profitLoss = (p - averageBuyPrice[stock.symbol]) * owned;
      ...
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${stock.symbol}</td><td>${owned}</td><td>$${p.toFixed(2)}</td><td>$${totalValue.toFixed(2)}</td>
...
window.sellAllStock = function(symbol){
  ...
  if(owned > 0){
    ...
    if(sellInput) sellInput.value = owned;
    sellStock(symbol);
  }
};
...
document.addEventListener('click', (e) => {
  if(e.target && e.target.id === 'add-watch'){
    ...
    if(sym && STOCKS.find(s => s.symbol === sym) && !watchlist.includes(sym)){
      watchlist.push(sym); renderWatchlist(); if(inp) inp.value=''; toast(`${sym} added to watchlist`);
    } else {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const openM = document.getElementById('open-missions');
  if(openM) openM.onclick = () => openModal('modal-missions');
  const closeM = document.getElementById('close-missions');
  ...
  if(saveBtn) saveBtn.onclick = () => { saveLeaderboardEntry(); toast('Score saved to local leaderboard'); };
