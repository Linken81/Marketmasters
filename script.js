// script.js - Missions fix & improvements
// - Per-symbol hold counters added to support "hold a stock for N ticks" missions.
// - Mission checks made robust and saved immediately when completed.
// - All other features preserved (XP/level, achievements, shop, watchlist, chart, ticks, news).
...
// Per-request behavior: reset level/xp and clear achievements/shop on refresh
state.level = 1;
state.xp = 0;
state.achievements = {};
state.shopOwned = {};
...
// ------------------ UI Helpers ------------------
function toast(text, timeout=3000){ const toasts = document.getElementById('toasts'); if(!toasts) return; const el = document.createElement('div'); el.className='toast'; el.textContent = text; toasts.appendChild(el); setTimeout(()=> el.remove(), timeout); }
function formatCurrency(v){ return `$${(+v).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`; }
function updateCash(){ const el = document.getElementById('cash'); if(!el) return; el.textContent = formatCurrency(portfolio.cash || 0); }
...
function addXP(amount){ if(amount<=0) return; if(state.activeBoosts.xpMultiplier) amount = Math.round(amount * state.activeBoosts.xpMultiplier); state.xp += Math.floor(amount); checkLevelUp(); saveState(); updateHUD(); }
function checkLevelUp(){ let gained=false; while(state.xp >= xpForLevel(state.level)){ state.xp -= xpForLevel(state.level); state.level++; gained=true; const rewardCoins = 50 + state.level*5; state.coins += rewardCoins; toast(`Level up! Now level ${state.level}. +${rewardCoins} coins`); launchConfetti(60); unlockAchievement('level_up'); } if(gained) saveState(); }
function updateHUD(){ const xpEl = document.getElementById('xp'); const lvlEl = document.getElementById('level'); if(xpEl) xpEl.textContent = state.xp; if(lvlEl) lvlEl.textContent = state.level; const bar = document.getElementById('xp-bar'); if(bar) { const pct = Math.min(100, Math.round((state.xp / xpForLevel(state.level)) * 100)); bar.style.width = pct + '%'; } updateCash(); renderNextAchievement(); }
...
function unlockAchievement(id){ if(state.achievements[id]) return; const spec = ACHIEVEMENT_LIST.find(a=>a.id===id); state.achievements[id] = true; if(spec){ state.coins += spec.coins; toast(`Achievement unlocked: ${spec.name} (+${spec.coins} coins)`); launchConfetti(80); } else { toast(`Achievement unlocked: ${id}`); launchConfetti(40); } saveState(); renderAchievements(); updateHUD(); }
function renderAchievements(){ const el = document.getElementById('achievements-list'); if(!el) return; el.innerHTML=''; ACHIEVEMENT_LIST.forEach(a=>{ const unlocked = !!state.achievements[a.id]; const div = document.createElement('div'); div.className='shop-item'; div.innerHTML = `<div><strong>${a.name}</strong><div style="font-size:0.9em;color:#9aa7b2">${a.desc}</div></div><div style="font-weight:700; color:${unlocked ? '#00fc87' : '#9aa7b2'}">${unlocked ? 'Unlocked' : 'Locked'}</div>`; el.appendChild(div); }); }
function renderNextAchievement(){ const el = document.getElementById('next-achievement'); if(!el) return; const next = ACHIEVEMENT_LIST.find(a => !state.achievements[a.id]); el.textContent = next ? `Next achievement: ${next.name} — ${next.desc}` : 'All achievements unlocked!'; }
...
const MISSION_CANDIDATES = [
  ...
  { id:'hold_10', text:'Hold a stock for 10 ticks', check:(p)=> { /* placeholder, we'll check holdCounters elsewhere */ return false; }, reward:{coins:80,xp:30} },
...
function generateDailyMissions(){ const today = getTodayStr(); if(state.missionsDate === today && state.missions && state.missions.length === 3) return; const shuffled = MISSION_CANDIDATES.sort(()=> Math.random()-0.5).slice(0,3); state.missions = shuffled.map(m => ({...m, done:false})); state.missionsDate = today; saveState(); }
function generateSingleMission(){ const activeIds = new Set((state.missions||[]).map(m=>m.id)); const pool = MISSION_CANDIDATES.filter(c=>!activeIds.has(c.id)); if(pool.length===0) return null; return {...pool[Math.floor(Math.random()*pool.length)], done:false}; }

function renderMissionsModal(){ const modalList = document.getElementById('missions-list'); if(!modalList) return; modalList.innerHTML=''; (state.missions||[]).forEach((m, idx)=>{ const div = document.createElement('div'); div.className='mission'; div.style.display='flex'; div.style.justifyContent='space-between'; div.style.alignItems='center'; div.innerHTML = `<div><strong>${m.text}</strong><div class="meta">${m.done ? 'Completed' : 'In progress'}</div></div><div>${m.done ? `<button class="action-btn" data-claim="${idx}">Claim</button>` : ''}</div>`; modalList.appendChild(div); if(m.done){ const btn = div.querySelector('button'); btn.onclick = ()=> { const reward = m.reward || {coins:50, xp:15}; state.coins += reward.coins || 0; addXP(reward.xp || 0); toast(`Mission claimed: +${reward.coins} coins, +${reward.xp} XP`); const newM = generateSingleMission(); if(newM) state.missions[idx] = newM; else state.missions.splice(idx,1); saveState(); renderMissionsModal(); renderMissionsBrief(); }; } }); }

function renderMissionsBrief(){ const el = document.getElementById('missions-brief'); if(!el) return; el.innerHTML=''; (state.missions||[]).slice(0,3).forEach(m=>{ const div = document.createElement('div'); div.textContent = `${m.text}${m.done ? ' ✅' : ''}`; el.appendChild(div); }); }
...
function renderShop(){ const el = document.getElementById('shop-items'); if(!el) return; el.innerHTML=''; SHOP_ITEMS.forEach(item=>{ const owned = !!state.shopOwned[item.id]; const div = document.createElement('div'); div.className='shop-item'; div.innerHTML = `<div><strong>${item.name}</strong><div style="font-size:0.9em;color:#9aa7b2">${item.desc}</div></div><div>${owned ? 'Owned' : `<button class="action-btn">Buy ${item.price}c</button>`}</div>`; el.appendChild(div); if(!owned){ const btn = div.querySelector('button'); btn.onclick = ()=> { if(state.coins >= item.price){ state.coins -= item.price; state.shopOwned[item.id] = true; applyShopEffect(item); toast(`Purchased ${item.name}`); saveState(); updateHUD(); renderShop(); } else toast('Not enough coins'); }; } }); }
...
function updateSeasonTimer(){ const el = document.getElementById('season-timer'); if(!el) return; const now = new Date(); const day = now.getDay(); const daysLeft = (7-day)%7; const end = new Date(now.getFullYear(), now.getMonth(), now.getDate()+daysLeft+1); const diff = end - now; const hrs = String(Math.floor(diff/3600000)).padStart(2,'0'); const mins = String(Math.floor((diff%3600000)/60000)).padStart(2,'0'); const secs = String(Math.floor((diff%60000)/1000)).padStart(2,'0'); el.textContent = `${hrs}:${mins}:${secs}`; }
...
function triggerRandomNews(){ const news = NEWS_EVENTS[Math.floor(Math.random()*NEWS_EVENTS.length)]; const el = document.getElementById('news-content'); if(el) el.textContent = news.text; const newsEffectMap = {}; if(news.type === 'stock') newsEffectMap[news.symbol] = news.effect; else if(news.type === 'type') STOCKS.forEach(s=>{ if(s.type===news.target) newsEffectMap[s.symbol] = news.effect; }); else if(news.type === 'market') STOCKS.forEach(s=> newsEffectMap[s.symbol] = news.effect); if(news.mood === 'good') addXP(5 + Math.round(Math.abs(news.effect)*100)); if(news.mood === 'bad') addXP(2); addEventToList(news.text); return newsEffectMap; }
...
window.buyStock = function(symbol){ const input = document.getElementById(`buy_${symbol}`); let qty = input ? parseInt(input.value,10) : 1; qty = Math.max(1, qty||1); const cost = (prices[symbol] || 0) * qty; if(cost <= portfolio.cash){ const prevQty = portfolio.stocks[symbol] || 0; const totalQty = prevQty + qty; averageBuyPrice[symbol] = (averageBuyPrice[symbol] * prevQty + (prices[symbol]||0) * qty) / Math.max(1, totalQty); portfolio.cash -= cost; portfolio.stocks[symbol] = totalQty; dayProgress.trades = (dayProgress.trades||0) + 1; if(!dayProgress.typesBought) dayProgress.typesBought = []; const type = (STOCKS.find(s=>s.symbol===symbol) || {}).type; if(type && !dayProgress.typesBought.includes(type)) dayProgress.typesBought.push(type); addXP(Math.max(1, Math.round(cost/200))); state.coins += Math.max(0, Math.round(cost/1000)); recordOrder('buy', symbol, qty, prices[symbol]); // FIRST TRADE ACHIEVEMENT if(!state.achievements || !state.achievements['first_trade']) unlockAchievement('first_trade'); toast(`Bought ${qty} ${symbol} for ${formatCurrency(cost)}`); saveState(); updateHUD(); updatePortfolioTable(); updateTradeTable(); updateStockTable(); renderWatchlist(); } else toast('Not enough cash'); };

window.sellStock = function(symbol){ const input = document.getElementById(`sell_${symbol}`); let qty = input ? parseInt(input.value,10) : 1; qty = Math.max(1, qty||1); const owned = portfolio.stocks[symbol] || 0; if(qty > owned){ toast('Not enough shares'); return; } const revenue = (prices[symbol] || 0) * qty; portfolio.cash += revenue; portfolio.stocks[symbol] = owned - qty; if(portfolio.stocks[symbol] === 0) averageBuyPrice[symbol] = 0; const profit = (prices[symbol] - averageBuyPrice[symbol]) * qty; if(profit>0){ addXP(Math.round(profit/10)); state.coins += Math.round(profit/50); dayProgress.dayProfit = (dayProgress.dayProfit||0) + profit; } recordOrder('sell', symbol, qty, prices[symbol]); if(!state.achievements || !state.achievements['first_trade']) unlockAchievement('first_trade'); toast(`Sold ${qty} ${symbol} for ${formatCurrency(revenue)}`); saveState(); updateHUD(); updatePortfolioTable(); updateTradeTable(); updateStockTable(); };

// ------------------ Order recording & watchlist ------------------
function recordOrder(type, symbol, qty, price){ orderHistory.unshift({ type, symbol, qty, price, ts: new Date().toISOString() }); if(orderHistory.length>200) orderHistory.pop(); }
function renderWatchlist(){ const wrap = document.getElementById('watchlist'); if(!wrap) return; wrap.innerHTML=''; watchlist.forEach(sym=>{ const div = document.createElement('div'); div.style.display='flex'; div.style.justifyContent='space-between'; div.style.alignItems='center'; div.style.marginBottom='6px'; const price = prices[sym] ? `$${prices[sym].toFixed(2)}` : '—'; div.innerHTML = `<div style="font-weight:700">${sym}</div><div style="color:#9aa7b2">${price} <button class="action-btn" data-remove="${sym}" style="margin-left:8px;">Remove</button></div>`; wrap.appendChild(div); const btn = div.querySelector('button'); btn.onclick = ()=> { watchlist = watchlist.filter(s=>s!==sym); renderWatchlist(); }; }); }
...
function tickPrices(){
  ...
  // update per-symbol hold counters: increment when owned, reset when not
  STOCKS.forEach(s=>{
  ...
  // update dayProgress.holdTicks as total ticks across holdings (kept for backward compat)
...
function checkMissions(){
  ...
  (state.missions || []).forEach(m => {
    ...
    // handle hold_10 specially using holdCounters
    if(m.id === 'hold_10'){
      const threshold = 10;
    ...
    // default: use m.check if provided (most missions have a check function)
    try {
      if(typeof m.check === 'function'){
        if(m.check(dayProgress)){ m.done = true; changed = true; }
      } else {
        // fallback checks for known mission ids
